...
import { createHouse } from "@/app/admin/adminControllers/calculatorController";

export default function CreateHouseTypeModal({ onClose }) {
  ...
  const handleAddAdmin = async () => {
    ...
    alert(`Saved house types: ${houseTypes.join(", ")}`);
    onClose(false);
  };
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4 p-2 border rounded-md">
          {houseTypes.map((type, index) => (
            ...
            >
              <span className="mr-2 text-[#595A5C] text-sm capitalize">{type}</span>
              <button
